...
<div id="osc-bindings-section" class="section {{'saved' if defined('saved') and saved else ''}}" data-fold-key="osc_bindings" data-help="osc_bindings">
 ...
 <span class="osc-binding-enabled-dot {{'on' if row.enabled else 'off'}}" aria-label="{{_('Enabled') if row.enabled else _('Disabled')}}"></span>
 ...
 <option value="{{d.id}}" {{'selected' if d.id == row.destination_id else ''}}>{{d.name or _('(unnamed)')}}</option>
 % end
 </select>
 <span id="destination-id-{{row.id}}-error" class="field-error"></span>
 <span class="field-note">{{_dest_label(row.destination_id)}} – {{_('manage in OSC Destinations')}}</span>
 </div>
 </div>
 </div>
